...
### 3.2. Compass & Sensor Fusion (LSM303AGR)
* **Hardware Output Data Rate (ODR) Configuration:**
  * Explicitly reconfigure the physical sensor registers on startup to eliminate hardware undersampling and capture high-speed motion:
    * Set the **LIS2MDL Magnetometer** ODR to **100Hz** using `setDataRate(LIS2MDL_RATE_100_HZ)`.
    * Set the **LSM303 Accelerometer** ODR to **200Hz** by writing `0x67` directly to register `CTRL_REG1_A` (address `0x20`) on I2C address `0x19`.
