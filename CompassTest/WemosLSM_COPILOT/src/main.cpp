...
// Higher values = fast & raw, Lower values = heavily dampened & smooth
float magAlpha = 0.15f;    // Keep the magnetometer/heading snappy and responsive (15% new data)
float accelAlpha = 0.025f; // Dampen the accelerometer to block impulses while keeping tilt snappy (2.5% new data)
bool firstRead = true;
...
void updateCompass() {
    ...
    // Direct angle filtering using a constant, balanced coefficient (0.030f)
    static float smoothed_pitch = 0.0f;
    static float smoothed_roll = 0.0f;
    static bool firstAngleFilter = true;
    const float pitchRollAlpha = 0.030f; // Constant 3.0% filter factor for highly stable yet snappy performance
    ...
    float Yh = cmx * sin(smoothed_roll) * sin(smoothed_pitch) + cmy * cos(smoothed_roll) - cmz * sin(smoothed_roll) * cos(smoothed_pitch);
