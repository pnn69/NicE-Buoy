...
QueueHandle_t ledNormalLed; // normal led
...
bool initledqueue(void)
{
    ...
    ledNormalLed = xQueueCreate(10, sizeof(LedData));
...
void LedTask(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(ledNormalLed, (void *)&ledNormal, 0) == pdTRUE)
        {
            if (ledNormal.blink == 0)
            {
            ...
            else
            {
                ...
                digitalWrite(BULETINLED, HIGH);
                ledTimer = ledNormal.blink + millis();
            }
        }

        if (ledTimer < millis() && ledNormal.blink != 0 && ledNormal.blink != -1)
        {
            digitalWrite(LED_PIN, LOW);
            digitalWrite(BULETINLED, LOW);
            ledNormal.blink = 0;
