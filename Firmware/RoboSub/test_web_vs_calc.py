...
while time.time() - start < 10:
    try:
        ...
        if msg.startswith(b'{'):
            ...
            try:
                resp = urllib.request.urlopen('http://192.168.1.78/data', timeout=1).read().decode('utf-8')
                magdir = json.loads(resp)['buoys'][0]['MagDir']
            except:
                magdir = "ERR"
                
            print(f"WEB_MAG: {magdir} | CALC_X: {hdg_x:05.1f} | CALC_Y: {hdg_y:05.1f} | CALC_Z: {hdg_z:05.1f}")
