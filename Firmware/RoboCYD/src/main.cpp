...
void update_nav_dynamic() {
    ...
    uint16_t udpDotColor = (millis() - last_udp_blink_ms < 300) ? TFT_GREEN : TFT_BLACK;
    tft.fillCircle(10, 13, 4, udpDotColor);
    
    uint16_t loraDotColor = (millis() - last_lora_blink_ms < 300) ? TFT_CYAN : TFT_BLACK;
    tft.fillCircle(230, 13, 4, loraDotColor);
    
    // --- 8. Update Live LoRa RSSI in Top-Left Corner of Header (Y: 5, X: 20) ---
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.setTextSize(2); // One step bigger (Size 2!)
    tft.setTextDatum(TL_DATUM);
    tft.setTextPadding(45); // Compact padding to prevent overwriting center ID!
    if (b.lora_rssi != -999) {
        sprintf(buf, "%d", b.lora_rssi);
    } else {
        sprintf(buf, "---");
    }
    tft.drawString(buf, 20, 5);
}
