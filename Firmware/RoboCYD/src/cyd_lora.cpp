...
void init_lora() {
    ...
    if (!LoRa.begin(LORA_BAND)) {
        ...
        xTaskCreatePinnedToCore(LoraRxTask, "LoraRxTask", 3072, NULL, 3, NULL, 0);
...
void send_lora_packet(const String &message) {
    ...
    if (loraMutex) xSemaphoreTake(loraMutex, portMAX_DELAY);
    LoRa.beginPacket();
    LoRa.write(message.length()); // Send the length byte first, as expected by RoboLora receivers!
    LoRa.print(message);
    LoRa.endPacket();
    if (loraMutex) xSemaphoreGive(loraMutex);
